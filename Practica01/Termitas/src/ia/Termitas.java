    ...
    class ModeloTermitas{
      ...
      */
      int direccionAleatoriaFrente(int direccion){
        ...
        int direccionGenerada;       
        int casos = rnd.nextInt(2);//Si le pongo 1 no funciona la evolución 2.
        if(rnd.nextBoolean()){
            direccionGenerada = (direccion + casos) % 8; 
        }else{
            direccionGenerada = (direccion - casos) % 8;
        }
      ...
      */
      boolean hayAstilla(Termita t, int dir){
         ...
         int tempX = t.posX;
         moverTermita(t, dir);
         boolean casillaSiguiente = mundo[t.posY][t.posX].estado;
         t.posY = tempY;
         t.posX = tempX;
      ...
      */
      void dejarAstilla(Termita t){
        ...
        mundo[t.posY][t.posX].estado = true;
        t.cargando = false;
        t.direccion = direccionAleatoriaFrente(rnd.nextInt(7));
        moverTermita(t, t.direccion);
      }
      ...
      */
      ...
      /**
       * Toma una casilla aleatoria del modelo en la que no haya astilla.
       * @param t Termina que va a soltar la astilla en el modelo.
       */
      private void asignarPosicion(Termita t){
          int x = rnd.nextInt(mundo[0].length);
          int y = rnd.nextInt(mundo.length);          
          if(!mundo[y][x].estado){
              t.posX = x;
              t.posY = y;
          }else{
