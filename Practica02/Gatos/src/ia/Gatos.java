        ...
        * Coloca la marca correspondiente, verifica y asigna la variable si hay un ganador.
        ...
        LinkedList<Gato> generaSucesores() {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    ...
                    if (nuevo.tablero[i][j] == MARCA1 || 
                            nuevo.tablero[i][j] == MARCA2) {
                        continue;
                    }
                    nuevo.padre = this;
                    nuevo.jugador1 = this.jugador1;
                    nuevo.tiraEn(j, i);
                    nuevo.jugador1 = !nuevo.jugador1;
        ...
        boolean esSimetricoDiagonal(Gato otro) {
            ...
            if ((tablero[0][0] != otro.tablero[2][2]) || 
                    tablero[0][1] != otro.tablero[2][1] ||
                    tablero[0][2] != otro.tablero[0][2] ||
                    tablero[1][0] != otro.tablero[2][1] ||
                    tablero[1][1] != otro.tablero[1][1] ||
                    tablero[1][2] != otro.tablero[0][1] ||
                    tablero[2][0] != otro.tablero[2][0] ||
                    tablero[2][1] != otro.tablero[0][1] ||
                    tablero[2][2] != otro.tablero[0][0]) {
                return false;
            }                                  
            return true;
        }
        ...
        boolean esSimetricoVerticalmente(Gato otro) {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (tablero[i][j] != otro.tablero[i][(j + 2) % (3 - j)]) {
                        return false;
                    }
                }
            }
        ...
        boolean esSimetricoHorizontalmente(Gato otro) {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    if (tablero[i][j] != otro.tablero[(i + 2) % (3 - i)][j]) {
                        return false;
                    }
                }
            }
